...
frames="${YOCTUI_WORKBENCH_PERF_FRAMES:-300}"
max_ns_per_frame="${YOCTUI_WORKBENCH_PERF_MAX_NS_PER_FRAME:-10000000}"
perf_target_dir="${YOCTUI_WORKBENCH_PERF_TARGET_DIR:-$repo_root/target/ui-performance}"
if [[ ! "$frames" =~ ^[1-9][0-9]*$ ]]; then
  printf '%s\n' 'YOCTUI_WORKBENCH_PERF_FRAMES must be a positive integer' >&2
  exit 2
fi
if [[ ! "$max_ns_per_frame" =~ ^[1-9][0-9]*$ ]]; then
  printf '%s\n' 'YOCTUI_WORKBENCH_PERF_MAX_NS_PER_FRAME must be a positive integer' >&2
  exit 2
fi
...
summary="$work_dir/workbench-ux.txt"
printf '%s\n' 'schema=yoctui.workbench-ux-performance.v1' >"$summary"
printf 'captured_utc=%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" >>"$summary"
printf 'terminal=160x48\nframes_per_scenario=%s\nmax_ns_per_frame=%s\n' \
  "$frames" "$max_ns_per_frame" >>"$summary"

for scenario in menu-heavy large-rootfs large-graph large-editor dense-terminal; do
...
mv "$summary" artifacts/profile/workbench-ux.txt
printf '%s\n' 'workbench UX performance thresholds passed'

...
def main() -> int:
    ...
    if args.scenario == "errors":
        ...
        expect(["Images", "core-image-minimal"], "image artifact inventory", 30)
        os.write(master, b"p")
        assertions = ["Rootfs composition", "Installed packages:", "Exact composition"]
        expect(assertions, "rootfs composition workflow", 120)
    else:
        interactions = [
            "press r to open Recipes",
            "type /busybox and Enter to select the real recipe",
            "press d and Y to run confirmed devtool modify",
            "press F10 to compose the application menu over the recipe editor",
        ]
        os.write(master, b"r")
        expect(["Recipes", "busybox"], "recipe inventory", 30)
        os.write(master, b"/busybox\r")
        expect(["busybox"], "busybox recipe selection", 10)
        os.write(master, b"d")

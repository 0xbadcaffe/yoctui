...
pub(super) fn handle(
    ...
    Ok(Some(match request.command {
                    ...
                    let event = services.daemon_journal.publish(
                        yoctui_protocol::daemon::DaemonEvent::JobChanged(
                            yoctui_protocol::daemon::JobSummary {
                                id: job_id,
                                kind: yoctui_protocol::daemon::JobKind::Qemu,
                                label: format!("QEMU session {session_id}"),
        ...
        DaemonCommand::CancelQemu { session_id } => {
            match services.qemu_supervisor.cancel(session_id) {

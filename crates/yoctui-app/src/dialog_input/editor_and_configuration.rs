pub fn recipe_editor_action(editor: &yoctui_model::RecipeEditor, key: Input) -> Option<Action> {
    ...
    if editor.focus == Focus::Files {
        return match key {
            ...
            Input::PageUp => Some(Action::SelectRecipeEditorFile { delta: -10 }),
            Input::PageDown => Some(Action::SelectRecipeEditorFile { delta: 10 }),
            Input::Home => Some(Action::SelectRecipeEditorFile { delta: isize::MIN }),
    ...
    match key {
        ...
        Input::Char('G') if editor.document.mode() != TextAreaMode::Insert => {

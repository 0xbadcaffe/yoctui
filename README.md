...
Run from an initialized environment: `source oe-init-build-env && cargo run -p yoctui -- core-image-minimal`. Use `--backend process` for the Knotty fallback, or `--headless` for scripts and CI. `yoctui doctor` explains environment issues.

...
class BitBakeAdapter:
    def __init__(self, version, family, module=None): self.version=version; self.family=family; self.module=module
    ...
    def server(self):
        ...
        if not callable(connector): raise ServerUnavailable("no supported BitBake server connector is available; start BitBake and expose bb.server.connect")
        try: return connector()
        except Exception as exc: raise ServerUnavailable(f"could not connect to the BitBake server: {exc}")
    def start_build(self, targets, task):
        connection=self.server(); operation=getattr(connection,"start_build",None)
        if not callable(operation): raise ServerUnavailable("connected BitBake server does not provide start_build")
        operation(targets,task)
    def cancel_build(self):
        connection=self.server(); operation=getattr(connection,"cancel_build",None)
        if not callable(operation): raise ServerUnavailable("connected BitBake server does not provide cancel_build")
        operation()
...
def select_adapter(version=None):
    ...
    if version is None:
        ...
        except ImportError: version=bitbake_version()
...
def handle(command,correlation_id,adapter):
    ...
    if kind=="hello": emit({"type":"hello_ack","bitbake_version":adapter.version},correlation_id)
        ...
        if not isinstance(targets,list) or not all(isinstance(t,str) and t for t in targets): error("invalid_request","start_build requires non-empty string targets",correlation_id)
            ...
            try: adapter.start_build(targets,command.get("task"))
            except ServerUnavailable as exc: error("bitbake_server_unavailable",str(exc),correlation_id)
            else:
        ...
        try: adapter.cancel_build()
        except ServerUnavailable as exc: error("bitbake_server_unavailable",str(exc),correlation_id)
        else: emit({"type":"build_completed","success":False},correlation_id)

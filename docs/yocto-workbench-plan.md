...
- [DONE] Add an on-demand dependency graph view backed by BitBake-generated graph data (`bitbake -g` or a supported server query), with bounded recipe/task topology, reverse/filter/expand navigation, why-built paths, exact provider/log jumps, and no independent dependency resolution. `A` requests the selected recipe's authoritative graph from a capable BitBake server.

...
Prepare v0.1.233, rerun the focused correction checks, build and install the
optimized binary, restart the daemon in the initialized Romulus environment,
and repeat provider inspection from a plain client plus embedded menuconfig
startup. Do not run the full workspace suite during the user's manual bug pass.

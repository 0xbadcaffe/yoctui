...
Adapt sstate readiness and protected cleanup.
...
| M6 Maintenance | IN_PROGRESS | The responsive contract and typed model are complete; exact sstate readiness and protected cleanup adapters are active |
...
| Maintenance workflow | IN_PROGRESS | The authoritative contract and first-class model now cover four views, exact bounded capability/metadata/operation identities, protected cleanup phrase, separate network confirmation, one correlated session lifecycle, replaceable evidence, cancellation, terminal outcomes, and routes to existing owned workspaces; sstate adapter implementation is active |
...
1. `MAINT-SSTATE-ADAPTER-001` — adapt sstate readiness and protected cleanup
2. `MAINT-SERVICE-ADAPTER-001` / `MAINT-RELEASE-ADAPTER-001` / `MAINT-OPTIONAL-ADAPTER-001` — adapt remaining exact tools and diagnostics

...
Route typed PR service export and import previews.
...
| Maintenance workflow | IN_PROGRESS | Sstate `c/d` runs through exact adapter previews and fresh cleanup rediscovery. Services `e/m` now exposes model-owned validated export/import forms with authoritative build/endpoint identity and native side-effect warnings; CLI preview routing is active. Release entry workflows remain. No live cache or service compatibility is claimed |
...
1. `MAINT-SERVICE-CLI-001` — route exact PR export/import previews

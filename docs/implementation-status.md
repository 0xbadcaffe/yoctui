...
Integrate SDK execution in the CLI.
...
| SDK workflow | IN_PROGRESS | The typed SDK model/app, authoritative artifact and shell-free tool adapters, and responsive SDK Workspace/Inspector/dialog rendering are complete. Every inventory/session state, exact preview, theme/no-color meaning, and 80x24 boundary is covered. Independent CLI polling/execution is next; fake scans/processes do not claim live SDK compatibility |
...
1. `SDK-CLI-001` — integrate SDK execution in the CLI
2. `SDK-UI-CLI-001` / `SDK-001` — close SDK integration gates

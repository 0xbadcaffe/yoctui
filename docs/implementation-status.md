...
Specify typed CVE and SPDX security workflows.
...
| M5 Testing/QA/Security | IN_PROGRESS | Unified Testing is complete; Security is split into specification, model, adapter, rendering, CLI, and parent gates; QA remains |
...
| Security workflow | IN_PROGRESS | Existing recipe shortcuts launch only authoritatively reported legacy CVE/SPDX tasks through managed BitBake and retain honest no-path outcomes. Typed CVE findings/package mappings, capability-driven legacy/current SBOM tasks, bounded report acquisition/viewing, a responsive Security workspace, and complete CLI integration remain |
...
1. `SEC-SPEC-001` — specify capability-driven CVE and SPDX workflows
2. `SEC-MODEL-001` — model typed security workflows
3. `SEC-ADAPTER-001` / `SEC-RENDER-001` — adapt and render security data

...
Adapt authoritative recipe and kernel QA capabilities.
...
| QA workflow | IN_PROGRESS | The authoritative contracts and complete typed QA model/app parent gate are done. Recipe/provider and configured-layer inventories, capability-supplied checks/vectors, indexed stale-safe managed/native operations, independent cancellation targets, bounded sessions/reports/findings, typed counts, exact opens, filters, drill, modal focus, and every terminal state are covered. Adapters, UI, and CLI integration remain |
...
1. `QA-TASK-ADAPTER-001` — adapt authoritative recipe and kernel QA capabilities
2. `QA-REPORT-ADAPTER-001` / `QA-LAYER-ADAPTER-001` — acquire QA evidence and execute layer QA
3. `QA-ADAPTER-001` — close the QA adapter gate
4. `QA-RENDER-001` / `QA-CLI-001` — render and integrate the complete QA workspace

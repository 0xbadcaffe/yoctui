...
| QA workflow | IN_PROGRESS | The authoritative contracts and complete typed QA model/app boundary are implemented. Recipe/provider and configured-layer inventories, capability-supplied checks/vectors, indexed stale-safe managed/native operations, independent cancellation targets, bounded sessions/reports/findings, typed counts, exact opens, filters, drill, modal focus, and every terminal state are covered. Parent model verification, adapters, UI, and CLI integration remain |
...
1. `QA-MODEL-001` — close the typed QA model gate
2. `QA-TASK-ADAPTER-001` / `QA-REPORT-ADAPTER-001` / `QA-LAYER-ADAPTER-001` — adapt QA execution and evidence

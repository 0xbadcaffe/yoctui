...
Run Wic creation and scan exact outputs.
...
| Wic workflow | IN_PROGRESS | Pure typed state and canonical bounded capability/kickstart/command construction are complete; cancellable creation execution and exact new-output scanning are active. The BitBake-only source has no live Wic installation |
...
1. `WIC-ADAPTER-RUNNER-001` — run Wic creation and scan exact outputs
2. `WIC-ADAPTER-001` — verify the complete Wic creation adapter

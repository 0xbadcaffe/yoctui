...
| Wic workflow | IN_PROGRESS | Cooked-mode creation plus protected-device discovery/model/rendering are complete: the Images workspace exposes exact identity, limitations, separate phrase/argv previews, write history/telemetry, responsive dialogs, and strong cancellation warning. Independently polled CLI execution remains; fake device/process coverage does not establish live Wic or removable-media compatibility |
...
1. `WIC-WRITE-CLI-001` — integrate protected device execution
2. `WIC-WRITE-UI-CLI-001` — verify protected device integration

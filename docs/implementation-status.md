...
Add typed PR service export and import forms.
...
| Maintenance workflow | IN_PROGRESS | Capability/diagnostic rendering and the independent runner are complete. Sstate `c/d` now runs end to end through exact readiness confirmation and non-destructive cleanup discovery before phrase confirmation, with fresh rediscovery before execution. PR service form work is active; release entry workflows remain. No live cache or service compatibility is claimed |
...
1. `MAINT-SERVICE-UI-001` / `MAINT-SERVICE-CLI-001` — add and route PR forms
2. `MAINT-RELEASE-UI-001` / `MAINT-RELEASE-CLI-001` — add and route release forms

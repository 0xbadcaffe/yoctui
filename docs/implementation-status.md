...
Adapt PR and hash service diagnostics.
...
| M6 Maintenance | IN_PROGRESS | The contract, model, and sstate adapter are complete; exact PR and hash service diagnostics are active |
...
| Maintenance workflow | IN_PROGRESS | The contract, first-class model, and sstate adapter are complete. Python/legacy cleanup discovery, exact readiness/cleanup vectors, bounded preview candidates, identity revalidation, and every runner outcome pass focused and baseline checks; service diagnostics are active |
...
1. `MAINT-SERVICE-ADAPTER-001` — adapt exact PR and hash service diagnostics
2. `MAINT-RELEASE-ADAPTER-001` / `MAINT-OPTIONAL-ADAPTER-001` — adapt remaining exact tools and diagnostics

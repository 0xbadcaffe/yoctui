...
| M7 Hardening | IN_PROGRESS | Reproducible fuzzing and deterministic high-volume/process-tree stress coverage join existing property, terminal, Valgrind, profiling, and Flamegraph foundations; sanitizer and final integration gates remain |
...
| Hardening matrix | IN_PROGRESS | Bounded fuzz targets cover protocol frames and retained logs; repeated deterministic stress tests cover 20,000 reducer events, 10,000 irregularly framed messages, and exact descendant termination through process-group cancellation. Dedicated sanitizer and final analysis integration remain |

## Priority queue

1. `HARDEN-SANITIZER-001` — add sanitizer verification
2. `HARDEN-ANALYSIS-001` / `HARDEN-001` — close the remaining hardening gates

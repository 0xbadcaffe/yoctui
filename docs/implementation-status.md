...
Acquire bounded QA findings and reports.
...
| QA workflow | IN_PROGRESS | The authoritative contracts, complete typed QA model/app parent gate, and fail-closed recipe/kernel capability adapter are done. Canonical recipe/provider/report-root identities and explicit task inventories drive the required check families without task or kernel guessing. Bounded report acquisition, layer execution, UI, and CLI integration remain; fixture evidence does not establish live compatibility |
...
1. `QA-REPORT-ADAPTER-001` — acquire bounded QA evidence
2. `QA-LAYER-ADAPTER-001` — execute exact layer QA

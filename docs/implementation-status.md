...
Add typed Wic creation and device-write state.
...
| Wic workflow | IN_PROGRESS | The parent is split into typed state, cooked-mode discovery/execution, responsive creation UI/CLI, independently revalidated device discovery/writing, and protected write UI/CLI children; the typed model child is active |
...
1. `WIC-MODEL-001` — add typed Wic creation and device-write state
2. `WIC-ADAPTER-001` — discover kickstarts and execute Wic creation safely
3. `WIC-UI-MODEL-001` — add Wic workspace dialog and input state
4. `WIC-UI-RENDER-001` — render responsive Wic creation and output UI
5. `WIC-UI-CLI-001` — integrate Wic creation in the CLI
6. `WIC-WRITE-ADAPTER-001` — discover and revalidate safe write devices
7. `WIC-WRITE-UI-CLI-001` — integrate protected device writing
8. `WIC-001` — verify the complete Wic parent gate

...
Inspect Wic capability and construct creation commands.
...
| Wic workflow | IN_PROGRESS | Pure typed state is complete; the creation adapter is split into capability/kickstart/command construction and runner/output-scanning children because the active BitBake-only source has no live Wic installation |
...
1. `WIC-ADAPTER-CAP-001` — inspect Wic capability and construct creation commands
2. `WIC-ADAPTER-RUNNER-001` — run Wic creation and scan exact outputs
3. `WIC-ADAPTER-001` — verify the complete Wic creation adapter
4. `WIC-UI-MODEL-001` — add Wic workspace dialog and input state
5. `WIC-UI-RENDER-001` — render responsive Wic creation and output UI
6. `WIC-UI-CLI-001` — integrate Wic creation in the CLI
7. `WIC-WRITE-ADAPTER-001` — discover and revalidate safe write devices
8. `WIC-WRITE-UI-CLI-001` — integrate protected device writing
9. `WIC-001` — verify the complete Wic parent gate

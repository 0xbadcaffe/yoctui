...
Render responsive QEMU launch and session UI.
...
| QEMU dialogs/session UI | IN_PROGRESS | Dialog/input state is complete; responsive launch/session rendering is the active child, followed by CLI-owned capability/runner integration |
...
1. `QEMU-UI-RENDER-001` — render responsive QEMU launch and session UI
2. `QEMU-UI-CLI-001` — integrate QEMU capability and runner in the CLI
3. `QEMU-UI-001` — verify the complete QEMU UI parent gate

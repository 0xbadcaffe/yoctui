...
Close the complete cross-layer Maintenance workspace gate.
...
| M6 Maintenance | IN_PROGRESS | The contract, model, adapters, responsive renderer, and non-blocking CLI execution are complete; the cross-layer gate is active |
...
| Maintenance workflow | IN_PROGRESS | The contract, model, adapters, responsive renderer, and non-blocking CLI coordinator are complete. Five CLI tests cover correlated partial discovery, unavailable tools, success/output, nonzero, timeout, exact cancellation, and unowned rejection; adapter coverage retains forced cancellation, loss, cleanup revalidation, and stale evidence. Full baseline passes without live fixture claims; the cross-layer parent gate is active |
...
1. `MAINT-UI-CLI-001` — close the cross-layer Maintenance gate
2. `MAINT-001` — close the Maintenance milestone parent gate

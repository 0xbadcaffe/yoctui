...
Compare loaded global and recipe-scoped configuration values.
...
| Configuration workspace | IN_PROGRESS | Live-validated metadata, searchable responsive detail, typed copy/source actions, and recipe-scoped inspection are complete; compare and previewed-edit actions remain |
...
1. `CONFIG-COMPARE-001` — compare typed configuration values
2. `CONFIG-EDIT-001` — add previewed configuration editing and refresh
3. `DEVTOOL-JOBS-001` — run Devtool operations as persistent jobs

...
currently launches lazy Kernel/U-Boot inspection without reconstructing the
selected Yocto environment, so an otherwise healthy daemon can leave provider
inspection loading before the bridge reports a missing `bb` module. Embedded
menuconfig also uses one fixed local socket, allowing a retry or concurrent
session to consume another relay's one-shot handoff and produce a broken pipe.
PLATFORM-INSPECTION-ENV-001, MENUCONFIG-RELAY-ISOLATION-001, and
PLATFORM-PTY-RELEASE-001 define the focused v0.1.233 correction series.
...
M75 GIT-WATCH-ACCESS-FILTER-001 is DONE in v0.1.219. Filesystem access events
generated by the read-only Git status command no longer retrigger the probe;
the badge stays stable while the repository is quiet and still refreshes after
a real worktree write.
...
as an error instead of returning an empty successful record.
...
input before pane focus. FIRMWARE-BRIDGE-NAV-001 is DONE in v0.1.199: Kernel
and U-Boot/BIOS inspection now lazily starts one daemon-authorized bridge for
the attached build, caches it for the client lifetime, and routes platform tabs
before pane focus. Focused app, model, CLI and UI checks pass across all four
bug series. Full-suite verification remains deferred until the user finishes
the agreed manual bug pass.
...
file set and rejects missing, changed or stale generated modules. All 32
categories, 464 commands, 288 executable entries and the reviewed reference
hash remain exact. Focused raw tests, generator/traceability checks, package
...
and seven correctness checks pass. M54 publication is DONE: all six crates are
published as v0.1.64, registry checksums match, and fresh install plus real-PTY
startup passed. The optimized release is installed locally with a backup; the
existing daemon was not restarted. Full completion passed with exit status 0. The optional
...
fresh v0.1.64 verification above. M54 publication is complete after full
completion passed and registry installation was verified. Its integration scope is image-owned udev rules, verification of existing tui-term
...
The seven-scenario pre-optimization release baseline is now retained with exact
raw samples and hashes. It measured 5.42% idle-daemon CPU, 2.43% attached idle,
...
by that broad completion label: the prior in-TUI RecipeEditor was append-only,
...
bounded-position state. Jump-to-log, source-open, and confirmation-gated rebuild
options remain truthful production actions, and the refreshed 160×50 cell/style
golden plus focused UI/model tests pass.
`UX-CONCEPT-ROOTFS-001` is complete: at the canonical width the production
...
| Dependency workspace | DONE | A linear-time bounded typed projection adds reducer-owned filtering, reverse anchors, collapse/expand, cycle/cross-edge and clipping reports, exact mouse rows, numeric positions, wide topology, medium tree, narrow table, ASCII/no-color and screen-reader parity while retaining authoritative recipe/provider/log actions |
...
| Terminal Sessions workbench | DONE | First-class navigator/menu/palette/prefix destination with typed contextual shell/devshell/menuconfig creation, tabs and client-local splits/zoom, current-replica writer leases, bounded copy/search/paste review/scrollback, conservative dropped-history evidence, rename/detach/close/confirmed kill, rich session identities, literal-prefix forwarding, and honest reconnect/exited/lost states. Ordinary writer characters remain PTY input. Focused model/app/UI/CLI tests plus a real daemon PTY cover input, viewport, resize, rename, termination, and close |

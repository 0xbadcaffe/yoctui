...
Integrate non-blocking Maintenance inspection and operation execution.
...
| M6 Maintenance | IN_PROGRESS | The contract, model, adapters, combined gate, and responsive renderer are complete; non-blocking CLI integration is active |
...
| Maintenance workflow | IN_PROGRESS | The contract, model, adapters, and responsive renderer are complete. Four Maintenance TestBackend tests cover all views, typed identities and limitations, lifecycle states, dialogs, breakpoints, themes, and no-color meaning; optional adapter details normalize into model-owned state. Full baseline passes without live fixture claims; non-blocking CLI integration is active |
...
1. `MAINT-CLI-001` — integrate non-blocking Maintenance execution
2. `MAINT-UI-CLI-001` — close the cross-layer Maintenance gate

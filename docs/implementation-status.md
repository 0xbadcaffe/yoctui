...
Adapt resulttool import, comparison, and export.
...
| Testing workflow | IN_PROGRESS | Typed launch/result state and the selftest runner adapter are complete. Canonical PATH-based tool discovery, exact revalidated argv, child-only network-test environment, bounded stream events, distinct timeout, all process terminal outcomes, and managed-BitBake task reuse pass fake-process and baseline checks. The resulttool adapter, complete rendering, and CLI execution remain; live compatibility is not claimed |
...
1. `TEST-RESULT-ADAPTER-001` — safe resulttool import/comparison/export
2. `TEST-RENDER-001` / `TEST-CLI-001` / `TEST-UI-CLI-001` / `TEST-001` — close integration gates

...
## M51 integrated editor regression

`artifacts/performance/editor/baseline.json` records the v0.1.56 failure of
the unchanged 10 ms/frame editor limit and a 2,144-sample CPU profile that
attributed 43.38% of self time to SHA-256. v0.1.57 replaces boolean dirty/diff
hashing with exact text comparisons, retaining hashes for revision authority.
`artifacts/performance/editor/result.json` records three quiet release repeats
and the passing full matrix (7.195 ms/frame for the editor), with exact source
and binary identity. It also preserves an unaccepted run overlapping profiling.
These are deterministic rendering fixtures, not real BitBake evidence.

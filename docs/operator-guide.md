...
| Logs | Follow bounded live output through the [tui-logger presentation](yocto-logs.md) and inspect exact selected records. | `f` follow, `w` wrap, `/` search, `n`/`N` matches, `s`/`R`/`T`/`B` filters, `o` source log, `C` copy when a clipboard helper is available. |
